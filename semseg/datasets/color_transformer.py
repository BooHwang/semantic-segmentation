...
def reinhard_color_transfer(target, source,target_p, source_p, clip=False, preserve_paper=False, source_mask=None, target_mask=None):
	"""
	...
	(lMeanSrc, lStdSrc, aMeanSrc, aStdSrc, bMeanSrc, bStdSrc) = lab_image_stats(src_input, source_p)
	(lMeanTar, lStdTar, aMeanTar, aStdTar, bMeanTar, bStdTar) = lab_image_stats(tgt_input, target_p)
...
def reinhard_color_transfer2(target, source, clip=False, preserve_paper=False, source_mask=None, target_mask=None):
	"""
	Transfers the color distribution from the source to the target
	image using the mean and standard deviations of the L*a*b*
	color space.
	...
	src_input = source if source_mask is None else source*source_mask
	tgt_input = target if target_mask is None else target*target_mask
	(lMeanSrc, lStdSrc, aMeanSrc, aStdSrc, bMeanSrc, bStdSrc) = lab_image_stats2(src_input)
	(lMeanTar, lStdTar, aMeanTar, aStdTar, bMeanTar, bStdTar) = lab_image_stats2(tgt_input)
...
def linear_color_transfer(target_img, source_img, mode='chol', eps=1e-5):
    '''
...
def lab_image_stats(image, mask):
    # compute the mean and standard deviation of each channel
    (l, a, b) = cv2.split(image)
    l = ma.array(l, mask=mask)
    a = ma.array(a, mask=mask)
    b = ma.array(b, mask=mask)
...
def color_transfer(ct_mode, img_src, img_trg):
    ...
    """
    img_src = (img_src / 255.0).astype(np.float32)
    img_trg = (img_trg / 255.0).astype(np.float32)
    if ct_mode == 'lct':
